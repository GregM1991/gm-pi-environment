...
Use a managed, reusable server instead of starting a new server for every review. This avoids dangling `http.server` processes when the skill is invoked repeatedly.

1. Create a stable canvas directory:
   ```bash
   CANVAS_DIR=/tmp/pr-review-canvas
   mkdir -p "$CANVAS_DIR"
   ```
2. Write your body HTML (everything that goes inside `<body>`) to `$CANVAS_DIR/pr-review-{number}-body.html`.
3. Save patches to `$CANVAS_DIR/pr-patches-{number}.json` using the `jq` command from step 3 above, adjusted for the canvas directory.
4. Run the Python assembly script from step 3 above (reads styles.css, renderer.js, template.html from this skill directory, injects body + patches safely, writes final HTML) and write the final page to `$CANVAS_DIR/pr-review-{number}.html`.
5. Start or reuse the managed canvas server. Always bind to this system's Tailscale IPv4 address so the printed URL is reachable from other machines:
   ```bash
   CANVAS_DIR=/tmp/pr-review-canvas
   PORT=${PORT:-8432}
   BIND_ADDR=${BIND_ADDR:-$(tailscale ip -4 | head -n1)}
   mkdir -p "$CANVAS_DIR"

   if [ -f "$CANVAS_DIR/server.pid" ] && kill -0 "$(cat "$CANVAS_DIR/server.pid")" 2>/dev/null; then
     echo "Reusing existing PR review canvas server"
   else
     rm -f "$CANVAS_DIR/server.pid"
     (
       cd "$CANVAS_DIR"
       python3 -m http.server "$PORT" --bind "$BIND_ADDR" \
         > "$CANVAS_DIR/server.log" 2>&1 &
       echo $! > "$CANVAS_DIR/server.pid"
     )
   fi

   echo "http://$BIND_ADDR:$PORT/pr-review-{number}.html"
   ```

   Navigate the in-app browser to the printed URL. If Tailscale is unavailable, explicitly set `BIND_ADDR=127.0.0.1` as a fallback before running the snippet.

   **Stopping the managed server:** only stop it when the user asks, or when you need to change bind address/port and cannot reuse the current process:
   ```bash
   CANVAS_DIR=/tmp/pr-review-canvas
   if [ -f "$CANVAS_DIR/server.pid" ]; then
     kill "$(cat "$CANVAS_DIR/server.pid")" 2>/dev/null || true
     rm -f "$CANVAS_DIR/server.pid"
   fi
   ```

   **Why a fixed port and managed directory:** Background shells have no TTY, so Python buffers its startup message ("Serving HTTP on...") indefinitely — using port 0 means you can never read which port was chosen. A shared `$CANVAS_DIR` plus PID file lets repeated invocations reuse one server while still writing unique files like `pr-review-135.html`, `pr-review-142.html`, etc. If port 8432 is taken by an unrelated process, try 8433, 8434, etc. and set `PORT` consistently for that server.
