...
<context>
This skill is the thinking process for adding durable observability. Not "add logs everywhere" — add the *right* signals at the *right* decision points.

Invocation points:
- Building loop/dispatch code with guards or retries
...
Use the project's existing logger if one exists. Prefer structured JSONL or equivalent structured events: one event per line/record, with `ts`, `event`, and domain-specific fields.
...
await writeAtomically(
  resolve("runtime/last-error.json"),
  JSON.stringify({
...
A fresh agent reading the runtime status directory sees what happened last, what was retried, and where the process stopped. Reuse any existing atomic-write helpers and project-standard runtime/status directories.
...
- **Digest view** — a small representation of recent work. In a server, this might be `/internal/status` with the last 10 request summaries; in a CLI, it might be a status file.
...
Remove noisy one-off instrumentation before finishing unless it provides durable diagnostic value.
...
- [ ] Failure state is persisted to a known location (`runtime/`, `/var/log/`, a status file).
