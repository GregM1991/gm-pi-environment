...
export function phasePrompt(phase: Phase, args: string, cwd: string, routingAddition?: string, conventionsContext = buildConventionsContext(cwd)): string {
	...
	const aiGateAutoLifecycle = aiGateCommand ? `\n9a. Before publication, read ${AUTO_REVIEW_LEDGER_REFERENCE}, then run the configured AI gate exactly once for this issue after its commit exists: \`${aiGateCommand}\`. Append its source-tagged outcome and update the same issue commit. PASS continues to publication. FIX or a concrete remediable BLOCKER enters a fix-worker plus fresh-review cycle while budget remains; use ${AUTO_CHILD_CONTRACTS_REFERENCE} for both children and ${AUTO_ARTIFACTS_REFERENCE} for verification validity. Update the issue commit normally after the passing follow-up review, without bypassing hooks, and never rerun the gate. If HEAD moved or unrelated work makes updating the commit unsafe, stop instead of rewriting ambiguous history. A non-remediable gate result or exhausted three-cycle budget stops without closing.` : "";
...
function helpText(): string {
	return [
		...
		"  /matt-auto [filter|parent]  Continuously implement, review, commit, publish, and close ready-for-agent issues; parent issues expand to child issues",
...
export default function mattWorkflowExtension(pi: ExtensionAPI) {
	...
	registerPhase("matt-auto", "auto", "Continuously implement, review, commit, publish, and close ready-for-agent issues", false, true);
