...
export function phasePrompt(phase: Phase, args: string, cwd: string, routingAddition?: string, conventionsContext = buildConventionsContext(cwd)): string {
	...
	const aiGateAutoLifecycle = aiGateCommand ? ` After the issue's review passes and its commit exists, run the configured AI gate exactly once for that issue, before closeout: \`${aiGateCommand}\`. Do not run it after review children. Capture its outcome separately with source: \`ai-gate\`, using the latest completed review cycle. Map no findings → \`PASS\`, actionable must-fix or should-fix findings → \`FIX\`, and execution/parsing failure or non-remediable blocking result → \`BLOCKER\`. A failure must append a blocking \`verification-skipped\` finding rather than omit evidence. Use the active implementation/fix worker skill pack on gate findings. Resolve every gate location to \`file:line\`; when output gives only a path, inspect the evidence and committed diff for the primary line, fall back to line 1, and use \`.pi/matt-conventions.json:1\` for a failure with no implicated file. Before appending, suppress any finding from that issue's review children matched by normalized location plus summary/evidence exactly as defined in the augmentation; append only novel gate findings and never label a duplicate-only gate result PASS. Classify each novel gate finding's repeat value under the unchanged finding-record rules. Any novel gate finding classified \`repeat: earlier-issue\` enters exactly the same recurring-class machinery as a review-child finding: match it by judgment to recurring classes already recorded this run and reuse a matching class's canonical key, or derive a fresh key under \`augmentations/auto.md\` only for a genuinely new class; inject the pitfall note into every remaining implementation and fix-child contract, file or reuse the prevention issue, and count it toward the prevention stop rule. Update the existing issue commit to include the appended gate ledger evidence. A gate \`FIX\` or concrete remediable \`BLOCKER\` triggers a fix worker and fresh review while fewer than three fix/review cycles have been used. The fix worker's completed full repo check satisfies the mandatory post-remediation/pre-commit verification requirement unless code or verification-relevant inputs change afterward; the fresh review and ledger bookkeeping do not invalidate it, so do not require a second identical complete check before updating the issue commit. Do not run the gate again. If a gate \`FIX\` or concrete remediable \`BLOCKER\` arrives after all three fix/review cycles have been consumed, stop with the budget-exhausted reason and do not close the issue. A non-remediable gate failure blocks closeout.` : "";
	...
	const prompts: Record<Phase, string> = {
		...
		auto: `${base}${routing}\n\nPhase: CONTINUOUS AFK AUTO-LOOP.\n\nTarget/filter: ${target}\n\nObjective: act as the parent orchestrator — continuously implement, review, commit, and close open, unblocked, ready-for-agent child/work issues, serially, until a stop rule fires. Use the applicable Matt upstream skill files above for issue queue state, worker contracts, review standards, and closeout decisions.\n\nHard constraints (tripwires; never trade these away):\n- Never implement a Wayfinder map/decision ticket; labels wayfinder:map and wayfinder:* always redirect to /matt-wayfinder and must be re-checked after every queue refresh.\n- Never implement or close a parent/spec/container issue; build the queue from its child issues. If parent detection is ambiguous, stop and report what needs human clarification instead of implementing the parent.\n- A milestone is not a parent issue; never infer spec/child hierarchy from shared milestone membership.\n- If the target/filter or selected issue involves a milestone, read ${MILESTONE_REFERENCE} before filtering the queue; stop if the documented relationship or readiness rules leave the queue ambiguous.\n- Work serially. Do not use parallel execution or worktrees unless the user explicitly asks for it in this run.\n- Never poll a running child with status/process checks or inspect git status, diffs, logs, or other repo state while that child is running. Wait for the child's returned result. If the harness emits an attention notice for a known-long verification command, continue waiting without taking investigative turns unless the harness reports the child failed or stalled. This does not restrict inspection after the child returns: then inspect its diff and verification evidence and append ledger records as required.\n- If the Pi subagent extension/tooling is available, use the builtin \`worker\` agent for implementation and fix children and the builtin \`reviewer\` agent for review children, always with \`context: \"fresh\"\`. Do not silently substitute a generic delegate, and do not make child agents run their own subagent workflows.\n- The parent orchestrator exclusively owns review launches, fix/review-cycle decisions, commits, and closeout. Implementation and fix children must return their diff and compact verification evidence without running /code-review, committing, closing issues, or launching subagents; this local contract overrides those final steps in upstream implement.\n- Full verification output must stay out of child handoffs and prompts. Store it at \`.pi/matt-verification/<issue>-<stage>.log\`; the three stage forms are \`<issue>-initial.log\`, \`<issue>-fix-<n>.log\` where \`<n>\` is the fix cycle number, and \`<issue>-pre-commit.log\`. Before running verification, ensure \`.pi/matt-verification/\` is ignored by Git, adding it to the repo-local \`.git/info/exclude\` when needed. Create \`.pi/matt-verification/\` with mode 0700 and log files with mode 0600, correcting existing modes before use. Delete that issue's logs after closeout and delete all verification logs created by this run whenever the loop terminates.\n- Route each selected issue before launching children. Stop before implementation/review on invalid routing config, missing selected routed skills, or high-confidence routed-skill overflow.\n- One commit per issue with a conventional commit message referencing the issue; do not combine multiple issues in one commit. Ledger appends belong in that same issue commit, never a separate commit.\n- Treat \`.pi/matt-review-ledger.jsonl\` as append-only and use the Review Ledger schema's generated \`--describe\` output for closed fields, taxonomies, and relationships; its append is expected loop work and must not trigger the dirty-worktree stop rule. Follow the semantic capture rules and append command in \`augmentations/auto.md\`. Append every new ledger record only through the validating command; never write a JSONL line directly. Treat a rejected append as a hard stop and report its specific reason instead of working around it. Every new record requires source: \`review-child\` or source: \`ai-gate\`; source-less legacy lines remain valid as \`review-child\` and must never be rewritten.\n- Per-issue review packets are temporary orchestration artifacts. Store them outside the worktree at \`\${TMPDIR:-/tmp}/matt-auto-review-packets/<repo-id>/<issue>.md\`, deriving the collision-safe sanitized \`<repo-id>\` exactly as defined in \`augmentations/auto.md\`; create directories with mode 0700 and packet files with mode 0600. Never stage or commit them. Their creation or modification is explicitly excluded from the dirty-worktree stop rule. Delete the issue packet after closeout and all packets created by this run whenever the loop terminates.\n- Close a child/work issue only when completion evidence supports it; otherwise do not close, relabel/recommend the correct next state, and stop.\n- Default limits unless the user explicitly supplied different ones: process at most 10 child/work issues and at most three fix/review cycles per issue. The initial implementation review does not consume this budget; each subsequent fix pass plus fresh follow-up review consumes one cycle.\n\nLoop contract (each iteration, in order):\n1. Before starting, inspect git status. If the worktree is dirty in a way not attributable to a just-finished loop iteration, stop and report it. Ignore the external per-issue review packet entirely when applying this dirty-worktree rule.\n2. Resolve the target/filter into a work queue:\n   - If no explicit target/filter is supplied, query open GitHub issues labeled ready-for-agent.\n   - If the target/filter names a specific GitHub issue or issue URL, inspect that issue and its comments first. If it is a parent/spec/container issue, discover its child issues and build the queue from those children.\n   - Treat an issue as a parent when it contains a spec, a child/sub-issues section, a task list of issue references, explicit 'parent', 'epic', or 'container' wording, or comments/metadata from ticket decomposition that identify child issues. Prefer explicit child issue references in the parent body/comments; otherwise use GitHub sub-issue metadata when available; only then fall back to linked issues or labels if the relationship is clear.\n3. Filter out all wayfinder:map and wayfinder:* planning artifacts, then filter the queue to open, unblocked, ready-for-agent child/work issues. Re-check Wayfinder classification after every queue refresh. Order oldest first unless dependency/blocker text says otherwise. Respect blocker labels and issue text such as 'blocked by #123', task-list dependencies, and acceptance criteria dependencies.\n4. Stop when there are no unblocked ready-for-agent issues in the active queue, when the next issue is labeled needs-info/ready-for-human/wontfix or otherwise requires human review, when label state conflicts, or when all child issues for a parent target are complete. If parent-targeted auto mode completes every child, stop and report that the parent workflow can continue to the next phase; leave the parent open.\n5. Route the selected issue, then, before launching any child for it, prepare its review packet at the external path defined above. The packet must contain: the fetched issue body and acceptance criteria; the parent/spec issue reference (or explicitly \`none\`); the complete routing contract and selected skill pack; references to relevant \`AGENTS.md\`, \`CONTEXT.md\`, ADRs, and other durable docs (or explicitly \`none found\`); and commands/paths for finding the current diff plus the compact verification summary, failing cases, and verification log path available for the active cycle. Update the same packet as diff and verification evidence changes across cycles.\n6. Launch a fresh implementation child with a concrete contract: give it the absolute review-packet path and require it to read the packet as provided context, then independently inspect the actual code and current diff rather than trusting the packet as a substitute for its own judgment. Implement exactly that child/work issue, read selected SKILL.md guidance first, keep scope minimal, use focused tests while editing, then run the complete repo check once when the implementation pass is complete. That completed full check also satisfies the mandatory pre-commit check if no code or verification-relevant inputs change afterward; rerun it only after such a change. A review result, ledger append, compact summary or review-packet update, and verification-log bookkeeping do not invalidate it. Redirect all verification output (stdout and stderr) to \`.pi/matt-verification/<issue>-initial.log\`. Require a handoff containing changed files and only the verification pass/fail summary, failing cases, and log path—never raw toolchain output. Explicitly tell the child not to review, commit, close issues, or launch subagents; those actions remain with this parent orchestrator.\n7. Launch a separate fresh review child against the issue, routing contract, implementation diff/commit, and compact verification evidence. Its contract must include the absolute review-packet path, require reading it as provided context, and still require independent inspection of the actual code and current diff. Give it the verification summary and log path, not raw logs, and explicitly permit it to read that repo-local log on demand. Require a pass/fix/blocker outcome and enough structured information to append records exactly as documented in \`augmentations/auto.md\`. After every review child returns (initial and each fix-cycle review), append one source: \`review-child\` \`.pi/matt-review-ledger.jsonl\` record per finding using that augmentation; for a PASS with no findings append one source-tagged verdict-only record. Add the current cycle and active worker skill pack to finding records, and classify whether each finding repeats an earlier cycle on this issue or an earlier issue. At append time, treat every \`repeat: earlier-issue\` finding as a guidance-promotion candidate. First compare it by judgment against recurring classes already recorded this run; if it matches, assign it to that class and reuse the class's key. Only for a genuinely new recurring class derive a fresh key with the unchanged normalization in \`augmentations/auto.md\`. Use the assigned canonical key verbatim for pitfall injection and the stop rule; embed it verbatim in and search for it in prevention issue bodies. From that moment, inject a short \`Known recurring pitfalls\` note naming the class and its concrete verification precaution into every remaining implementation and fix-child contract in this run (prompt-only; change no file). For each newly detected recurring class, search the target repo tracker for an existing open prevention issue; if none exists, file exactly one human-triage prevention issue citing each motivating ledger record by issue, cycle, category, and \`whyMissed\`, and name candidate prevention tiers: target-repo \`AGENTS.md\` guidance, a routed skill, or, strongest, a deterministic toolchain check. Deduplicate by recurring class, record the existing or newly filed prevention issue, and never apply any durable guidance, skill, routing, or test-policy change in the loop itself. A first \`earlier-issue\` repeat only injects and schedules prevention; it never stops the loop. Stop only if the same recurring class appears again later in this run after that class's pitfall note was injected; report the class and its prevention issue rather than consuming another fix cycle.\n8. Classify the review-child outcome before deciding whether to stop. A FIX or BLOCKER verdict with concrete repo-local changes is fixable and must continue while fewer than three fix/review cycles have been used: launch a fix worker for the cited findings. Its contract must include the absolute review-packet path, require reading it as provided context, and still require independent inspection of the actual code and current diff. It also requires focused tests during intermediate edits, then one complete repo check after that fix cycle is complete, with all output redirected to \`.pi/matt-verification/<issue>-fix-<n>.log\`, where \`<n>\` is the fix cycle number; it returns only pass/fail summary, failing cases, and log path. Increment the cycle count, update the packet with current diff and verification evidence, then launch a separate fresh reviewer under the same packet and independent-inspection requirements against the updated diff and allow it to read that log on demand. Do not stop merely because a reviewer labels a concrete correctness finding as BLOCKER. Repeat through cycle 3 when concrete findings remain. Stop before exhausting the budget only when the finding requires a product/design decision or other human judgment, acceptance criteria are unclear, verification cannot pass, or merge/conflict risk makes an automatic fix unsafe. After cycle 3, stop if the fresh review still does not pass and report that the three-cycle budget was exhausted.\n9. After review passes, ensure a completed full repo check covers the exact code and verification-relevant inputs to be committed. The implementation/fix-cycle full check satisfies this mandatory pre-commit check when neither changed afterward. A routine review result, ledger append, compact summary or review-packet update, or verification-log bookkeeping after that check does not trigger another check; proceed directly to commit. Rerun the complete check immediately before committing only after actual remediation, code changes, or other verification-relevant input changes. Redirect its full output to \`.pi/matt-verification/<issue>-pre-commit.log\` and keep only its summary, failing cases, and log path in the conversation. If the complete pre-commit check fails, re-enter the fix/review cycle while fewer than three cycles have been used; otherwise stop with the budget-exhausted reason. Never commit on a failing check. Create the commit for that issue only if the check passes and there are uncommitted changes for it.${aiGateAutoLifecycle}\n10. Run closeout logic for that issue: post an AI-generated completion comment using the triage disclaimer, summarize changes and verification, and close only when evidence supports it.\n11. Re-check dependencies and issue state after each closeout, then continue serially with the next unblocked ready-for-agent issue until a stop rule fires.\n\nDone when the loop has stopped (self-check before finishing):\n1. A stop rule or default limit fired and the exact reason is stated.\n2. The final response contains a compact loop log: parent issue if any, child/work issues completed, commits, verification, fix/review cycles used and ledger records appended per source per issue (including per-issue AI-gate duplicates suppressed across review cycles), issues skipped, and the exact blocker/stop reason. It also contains a \`Guidance-promotion candidates\` section. If there are no candidates, this section states \`none\`; otherwise it lists every \`earlier-issue\` repeat with its recurring class, category, and issue/cycle ledger references, prevention issues filed or reused, and whether the prevention stop rule fired.`,
	};
