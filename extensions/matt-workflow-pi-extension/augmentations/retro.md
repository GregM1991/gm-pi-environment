...
Call out `ai-gate` `verification-skipped`/`BLOCKER` records as gate reliability failures, not worker correctness findings. A cycle with only suppressed same-cycle gate duplicates has no AI-gate ledger record and is therefore outside the recorded pass-rate denominator; do not infer a PASS from absence.
...
4. Concrete target and change kind: add/edit/delete a target-repo `AGENTS.md`; edit an extension-local augmentation; edit `.pi/matt-skill-routes.json`; or create a new skill.
5. The exact intended change at a level the user can approve or reject.
6. Rationale citing the motivating ledger records by source, issue, cycle, and category.
