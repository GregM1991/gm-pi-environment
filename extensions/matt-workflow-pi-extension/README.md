...
- `/matt-auto [filter|parent]` — continuously implement, review, commit, publish, and close ready-for-agent issues until blocked; when passed a parent/spec issue, expands it into child issues and stops after the child queue is complete
...
When `toolchain.commands.aiGate` is configured, auto mode runs it once per issue after the issue commit and before publication. The focused Review Ledger reference owns outcome mapping, same-issue deduplication, recurrence, and remediation behavior. Any remediation updates the issue commit before it is pushed; user-invoked `/matt-review` keeps its existing in-review gate behavior.
