...
A project-owned `.pi/matt-conventions.json` sibling to `.pi/matt-skill-routes.json` that records repo conventions used in Matt workflow base prompts: tracker labels doc, toolchain/runtime commands, workflow doc, and extra context docs. The format is strict JSON with required `version: 1`. Each section is optional; omitted sections fall back to repo detection for that section only. If the file is absent, current detection runs. If the file exists but is invalid, every command that injects base context hard-stops with diagnostics before sending a phase prompt. Repo Conventions Config is independent from Repo Skill Route Config so conventions can evolve without changing routing schema.
_Avoid_: settings file, skill routes config, routing policy, all-or-nothing fallback, silent fallback on broken config
