...
A project-owned `.pi/matt-conventions.json` sibling to `.pi/matt-skill-routes.json` that records repo conventions used in Matt workflow base prompts. Version 1 defines the original hints, version 2 adds delivery policy, and version 3 makes each extra context document a Branch-scoped Context Document while preserving earlier versions unchanged. Invalid config hard-stops context-injecting commands; absent sections retain detection fallback.
...
**Branch-scoped Context Document**:
An extra repo document paired with the workflow branch that requires reading it, represented by a repo-relative `path` and a non-empty `useWhen` condition in Repo Conventions Config version 3.
_Avoid_: unconditional context dump, bare extra-context path
